...
;10-10 OpAdd
 ...
 OpAdd       JSR     POP             ; Get first source operand.
             ...
             ADD     R5,R5,#0        ; Test if POP was successful.
             BRp     RestoreAdd1        ; Not successful, put back first.
             ADD     R0,R0,R1        ; THE Add.
             JSR     RangeCheck      ; Check size of result.
             BRp     RestoreAdd2        ; Out of range, restore both.
             JSR     PUSH            ; Push sum on the stack.
             BRnzp  ExitAdd                     ; On to the next task...
RestoreAdd2     ADD     R6,R6,#-1       ; Decrement stack pointer.
RestoreAdd1     ADD     R6,R6,#-1       ; Decrement stack pointer.
...
;10-10 OpMod
 ...
 OpMod       AND     R2,R2,#0
             ...
             ADD     R5,R5,#0        ; Test if POP was successful.
             BRp     ExitMod         ; Branch if not successful.
             ADD     R0,R0,#0
             BRn     RestoreMod1
             ADD     R1,R0,#0        ; Make room for second operand
             JSR     POP             ; Get second source operand.
             ADD     R5,R5,#0        ; Test if POP was successful.
             BRp     RestoreMod1     ; Not successful, put back first.
             ADD     R0,R0,#0
             BRn     NegNum          ; Add if R0 is negative and minus if R0 is positive
             NOT     R1,R1
             ADD     R1,R1,#1         ; 2's com of R1
LoopMod      ADD     R2,R1,R0
             BRn     EndMod
             ADD     R0,R0,R1
             BRnzp   LoopMod
NegNum       ADD     R0,R0,R1
             BRn     NegNum
EndMod       JSR     RangeCheck      ; Check size of result.
             BRp     RestoreMod2        ; Out of range, restore both.
             JSR     PUSH            ; Push sum on the stack.
             BRnzp  ExitMod                     ; On to the next task...
RestoreMod2     ADD     R6,R6,#-1       ; Decrement stack pointer.
RestoreMod1     ADD     R6,R6,#-1       ; Decrement stack pointer.
ExitMod         BRnzp	NewCommand
...
OpXOR        AND     R2,R2,#0
             ...
             JSR     POP             ; Get first source operand.
             ADD     R5,R5,#0        ; Test if POP was successful.
             BRp     ExitXOR         ; Branch if not successful.
             ADD     R1,R0,#0        ; Make room for second operand
             JSR     POP             ; Get second source operand.
             ADD     R5,R5,#0        ; Test if POP was successful.
             BRp     RestoreXOR1     ; Not successful, put back first.
             NOT     R2,R0
             AND     R2,R2,R1
             NOT     R3,R1
             AND     R3,R3,R0
             ...
             NOT     R0,R2
             JSR     RangeCheck      ; Check size of result.
             BRp     RestoreXOR2        ; Out of range, restore both.
             JSR     PUSH            ; Push sum on the stack.
             BRnzp   ExitXOR                     ; On to the next task...
RestoreXOR2  ADD     R6,R6,#-1       ; Decrement stack pointer.
RestoreXOR1  ADD     R6,R6,#-1       ; Decrement stack pointer.
ExitXOR      BRnzp	 NewCommand
...
OpMult         AND    R3,R3,#0      ; R3 holds sign of multiplier.
               ...
               BRp    ExitMult          ; Failure
               ADD    R1,R0,#0      ; Make room for next POP
               JSR    POP           ; Get second source operand
               ADD    R5,R5,#0      ; Test for successful POP
               BRp    RestoreMult1      ; Failure; restore first POP
...
PushMult       JSR    PUSH          ; Push product on the stack.
               BRnzp ExitMult
RestoreMult2       ADD    R6,R6,#-1     ; Adjust stack pointer.
RestoreMult1       ADD    R6,R6,#-1     ; Adjust stack pointer.
ExitMult           BRnzp	NewCommand   
...
;  the PUSH operation.
;
PUSH           ST      R1,SavePUSH1      ; R1 is needed by this routine
               LEA     R1,StackMax
               NOT     R1,R1
               ...
               BRnzp   Success_exit
Overflow       ST      R7,SavePUSH2
               LEA     R0,OverflowMsg
               PUTS
               LD      R7,SavePUSH2
               LD      R1, SavePUSH1     ; Restore R1
               AND     R5,R5,#0    
               ADD     R5,R5,#1      ; R5 <-- failure
               RET
Success_exit   LD      R1,SavePUSH1      ; Restore R1
               AND     R5,R5,#0      ; R5 <-- success
               RET
SavePUSH2          .FILL   x0000
SavePUSH1          .FILL   x0000
